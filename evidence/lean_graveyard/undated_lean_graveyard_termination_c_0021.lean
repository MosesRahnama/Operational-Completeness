...
@[simp] lemma kappa_drop_recSucc (b s n : Trace) :
  ...
  -- κ(merge …) = 0; κ(recΔ … (delta n)) = succ (κ (delta n)) = 1
  simp [kappa]
...
-- (moved dependent lex lemmas after μ-decrease lemmas to avoid forward refs)
...
set_option diagnostics true
-- Sound termination proof for R_rec_succ -/
...
--- DO NOT USE ANYTHING AFTER HERE- DEAD END!---------
