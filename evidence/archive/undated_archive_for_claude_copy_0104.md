...
Build sequence
- Make the R_int_delta replacement; lake build.
- Replace R_merge_* branches; lake build.
- Replace R_rec_zero; lake build.
- Replace R_rec_succ with split + μ lemma; lake build.
- Add Eq rules; lake build.
- Remove diagnostics settings; lake build.

That’s it. This keeps Claude_SN strictly structural-κ + μ-lex and green, and does not change plan or spill into other files.