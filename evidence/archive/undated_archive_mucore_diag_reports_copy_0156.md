...
- Root causes:
  ...
  - Exponent-shape mismatch: trying to match `ω^3 * (x+1)` with `ω^x * ω^4` or `ω^(x+4)` without a valid exponent inequality.
  - A few stray tactics or placeholders in the earlier draft (e.g., “unknown tactic” from numeric tactics on ordinals).
- What works already:
  - Using ω-normality (`isNormal_opow.strictMono`) and the principal-add lemma (`principal_add_omega0_opow`) to aggregate three terms under a single principal bound. This is the right pipeline for `mu_lt_eq_diff`.
- Fix blueprint:
...
- Fix:
  ...
  - Refer to μ as `MetaSN.mu` everywhere in `MuCore.lean` (or add `local notation "μ" => MetaSN.mu` and replace plain `mu` with `μ`).
...
- Symptom (early traces):
  - Target wants `ω^x * ω^4` or `ω^(x+4)`; simp produces `ω^3 * (ω^x * ω)`; mismatch remains.
  - “type mismatch, term this… has type ω^3 * (ω^x * ω) ≤ … but expected ω^x * ω^4 …”
  - “add_comm”/“add_left_comm” attempts fail (ordinal addition is not commutative).
- Correct pipeline for each helper:
  - Start from hx: x+1 ≤ ω^(x+1), obtained by right-le-opow at base ω: `Ordinal.right_le_opow one_lt_omega0`.
  - Multiply by left-positive: `mul_le_mul_left' hx (omega0 ^ n)` (n = 3 or 2).
  - Rewrite RHS via opow_add: `ω^n * ω^(x+1) = ω^(n + (x+1))`.
  - Now raise exponent inequality using `Ordinal.opow_le_opow_right` (requires 0 < ω) if you can show `n + (x+1) ≤ x + (n+1)`.
  - Bridge `n + (x+1) ≤ x + (n+1)` safely (do not use add_comm):
    - Use a small local ordinal lemma that handles finite-vs-infinite x:
      ...
      - If ω ≤ x (infinite), use `add_one_eq_self (hω : ω ≤ x)` to simplify `(x+1)` to `x` on the right, then monotone-add-on-the-left plus known facts about finite shifts to get `n + (x+1) ≤ x + (n+1)`.
  - This removes the need to rely on invalid `add_comm` for ordinals.

3) Principal-add aggregation is correct; keep it
- You already use:
  - Strict monotonicity of ω-exponent: `((isNormal_opow one_lt_omega0).strictMono h)`.
...
- This pipeline is the right (and safe) way to prove `mu_lt_eq_diff`:
  - Bound each payload piece below `ω^(C+5)`, combine them using principality, then multiply by `ω^4`, collapse via `opow_add`, and bridge the final +1’s.
...
- Use of `principal_add_omega0_opow` to bound triple-sums under a single ω-principal is exactly the safe tool.
...
- The final “expand μ-left/right then simpa” pattern is fine once μ is properly qualified and earlier inequalities are fixed.
...
- Imports and μ alias:
  ...
  - In `namespace MuCore`, add either:
    ...
    - `local notation "μ" => MetaSN.mu` and rewrite the file to use `μ`.
- termA_le:
  - Current “simpa … [add_comm]” is not valid; replace with:
    - Multiply hx by `ω^3`, rewrite via `opow_add`, then apply `Ordinal.opow_le_opow_right` using a small lemma `add3_plus1_le_plus4`.
- termB_le:
  - Same refactor with `ω^2` and a lemma `add2_plus1_le_plus3`.
...
- μ-drop core (present; keep this shape):
...
```
Replace the last line with: rewrite to `ω^(3+(x+1))`, then apply `Ordinal.opow_le_opow_right` using `add3_plus1_le_plus4 x`. Do not use `add_comm` on ordinals.
...
Next steps
- Implement the μ-qualification and the two safe helper lemmas.
...
- This restores the μ-drop lemma for `eqW_diff`, enabling the already-working lexicographic SN harness.
...
MuCore.lean is the μ-provider that Termination Lex imports to finish the complete strong–normalisation proof.
...
Earlier μ-drop lemmas (mu_void_lt_integrate_delta …, … mu_lt_rec_base) are proven elsewhere and re-export without incident.
...
Drop the simp? shotgun – replace with the three-step pattern already validated in Termination_C.mu_lt_eq_diff (inner-bound → multiply by ω⁴ → collapse with opow_add).

Guard against absorption hazards – before folding sums, establish omega0 ≤ ω^(C+5) via opow_le_opow_right; this satisfies AP1-AP3 in the toolkit.
...
|---------|--------------|---------|------------|
| A | 33, 40, 332 | `ω ^ 3 * succ x ≤ …` vs `ω ^ x * ω ^ 4` | RHS written in non-canonical product form; needs `opow_add` collapse to `ω^(x+4)` (same for *x+3*). |
...
1. **termA_le / termB_le**
   * `mul_le_mul_left'` produces `ω^3 * (x+1) ≤ ω^3 * ω^(x+1)`.
   * Rewrite RHS with `opow_add` → `ω^(3+(x+1)) = ω^(x+4)`.
...
4. **Principal-add application**
   * `principal_add_omega0_opow` expects *strict* bounds for each summand under a common ω-tower. After A–D, `hsum` goals satisfy its premises.
...
4. Re-run build; remaining goals should reduce to κ-side equalities already solved in Termination_Lex.
