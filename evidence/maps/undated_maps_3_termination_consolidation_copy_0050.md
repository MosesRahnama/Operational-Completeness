...
### 6.1 Core Strong Normalization Cases Status (Legacy μ-only track)
...
### 6.1b KO7 SafeStep and Computable SN (Adopted triple-lex)

We have adopted a triple-lexicographic measure that resolves the rec_succ case cleanly:

- μ3 (KO7): (δ, κᴹ, μ) where δ = `deltaFlag`, κᴹ is a DM multiset of recursion weights, μ is the ordinal from the legacy track.
- μ3c (Computable): (δ, κᴹ, τ) where τ is a computable Nat rank replacing μ.

References (Lean files and lemmas):
- KO7 measure and decreases: `OperatorKernelO6/Meta/Termination_KO7.lean`
  - `MetaSN_KO7.deltaFlag` (δ)
  - `MetaSN_DM.kappaM` (κᴹ, DM-based)
  - rec-succ handled by δ-left: `MetaSN_KO7.drop_R_rec_succ`
  - per-rule decreases aggregated: `MetaSN_KO7.measure_decreases_safe`
  - well-foundedness: `MetaSN_KO7.wf_SafeStepRev`
- Computable variant: `OperatorKernelO6/Meta/ComputableMeasure.lean`
  - τ, μ3c, Lex3c, DM lifting: `tau`, `mu3c`, `Lex3c`, `dm_to_LexDM_c_left`
  - rec-succ δ-left: `drop_R_rec_succ_c`
  - all 8 decreases: `measure_decreases_safe_c`
  - well-foundedness: `wf_SafeStepRev_c`

Implications:
- rec_succ no longer requires any global ordinal domination; δ drops from 1 to 0 in KO7.
- κ-duplication cases are oriented via DM on κᴹ; where κᴹ ties at 0, μ (or τ) provides the strict drop.
- The computable path (μ3c) is fully sorry-free and covers all 8 SafeStep constructors.

Scope notes:
- KO7 theorems are scoped to `SafeStep` and do not claim full `Step` confluence/SN without guards. This is intentional and reflected in `Confluence_Safe.lean`.
