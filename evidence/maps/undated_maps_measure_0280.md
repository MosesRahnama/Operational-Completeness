...
| _, _, Step.R_eq_refl a => by
    ...
    · apply drop_right (MetaSN.mu_void_lt_eq_refl a)
      simpa [kappa_eqW, kappa_void, Nat.max_self, h]
...
| _, _, Step.R_eq_diff (a:=a) (b:=b) hneq => by
  ...
  simpa [kappa_eqW, kappa_integrate, kappa_merge, Nat.max_comm, Nat.max_assoc, Nat.max_left_comm]

/-- The reverse step relation (for forward normalization). -/
def StepRev : Trace → Trace → Prop := fun a b => Step b a

/-- Strong normalization: the step relation is well-founded in reverse. -/
