...
private theorem nonvoid_mu_ge_omega {t : Trace} (h : t ≠ .void) :
  ...
  | app a b =>
      -- μ(app a b) is definitionally equal to μ(merge a b); reuse the merge proof.
      have hmerge : omega0 ≤ MetaSN.mu (.merge a b) := by
        -- `.merge a b` is never void, so we can apply the existing lemma.
        have h_nv : (merge a b) ≠ .void := by
          intro hvoid; cases a <;> cases b <;> cases hvoid
        simpa using nonvoid_mu_ge_omega h_nv
      simpa [MetaSN.mu] using hmerge
...
/-- Combined measure strictly decreases for every kernel rule (reverse orientation). -/
...
theorem strong_normalisation :
  WellFounded (fun a b : Trace => OperatorKernelO6.Step b a) := by
  -- Well-foundedness of the lexicographic product (Nat, Ordinal)
  have wf_measure : WellFounded LexNatOrd :=
    WellFounded.prod_lex Nat.lt_wfRel.wf Ordinal.lt_wf
  -- build inv-image on μκ and wrap with our decrease lemma
  exact
    Subrelation.wf (by
        intro x y hxy
        exact μκ_decreases hxy)
      (InvImage.wf (f := μκ) wf_measure)
